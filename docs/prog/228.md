...
> 原文:[https://www . tutorialgateway . org/python-numpy-string-functions/](https://www.tutorialgateway.org/python-numpy-string-functions/)

Python numpy 字符串函数用于根据您的要求更改给定的字符串。numpy 字符串函数有:加法、乘法、大写、标题、上、下、居中、拆分、分割线、剥离、连接、替换、编码和解码。例如，numpy 字符串上部函数转换为大写。这些示例帮助您理解 Python numpy 字符串函数。
...
用于字符串连接的 python numpy add 函数。在这个例子中，我们使用这个 numpy add 函数来添加两个字符串
...
Python numpy 乘法函数用于将给定的字符串相乘或重复指定的次数。在这个例子中，p.char.multiply('hello '，2)重复 hello 两次。
...
![Python Numpy multiply Function](img/3eac11c1efb22c161704dc22d04fad4b.png)

## Python numpy 大写

Python numpy 大写函数用于大写完整字符串中的第一个字母。
...
![Python Numpy capitalize Method](img/a0e19e0d1bf2d9349f1e39806eb880ca.png)

## python num py title(python num py 标题)

Python numpy title 函数是将每个单词的第一个字母转换成大写
...
![Python Numpy title Function](img/ca3ceca8facf936f01cdb241c1e31c2e.png)

## Python numpy 上部

Python numpy upper 将字符串中的所有字母都转换为大写
...
![Python Numpy upper Function](img/18ed4a517174f0bb8bc1254837e822b8.png)
...
![Python Numpy lower Function](img/af614831850f515372f4802e49d5e3c1.png)

## Python numpy 中心

Python numpy 中心用于填充字符串。 [Numpy](https://www.tutorialgateway.org/python-numpy-array/) 中心的语法
...
Python Numpy 条从给定字符串的右侧和左侧移除空格。
...
Python numpy 分割函数根据指定的分隔符分割给定的字符串
...
这个 Python numpy splitlines 返回给定字符串中的行列表。numpy 分隔线考虑了换行符\n 和\r。
...
Python numpy join 函数在给定字符串的每个字符后连接指定的字符(第一个参数)。
...
Python numpy 替换函数用于用新字符串替换子字符串
...
Python numpy 编码用于使用指定的编码器对字符串进行编码。这里，np.char.encode('Hello '，' cp500 ')使用 cp500 对 Hello 字符串进行编码
...
## Python numpy string 解码

Python numpy 解码用于解码已经编码的(numpy encode)字符串。在这里，您必须指定用于解码消息的代码。如果您使用了错误的代码，那么 numpy decode 函数会抛出一个错误。
