...
我们可以用多种方法初始化 C 二维数组。在此之前，请参考 [C 编程](https://www.tutorialgateway.org/c-programming/)中的[数组](https://www.tutorialgateway.org/array-in-c/)和[多维](https://www.tutorialgateway.org/multi-dimensional-array-in-c/)文章
...
以上三种在 C 语言中初始化二维数组的方式，都很好的将少量元素存储到数组中。如果我们想要存储 100 行或 50 列的值会怎么样。使用上面提到的任何一种方法添加所有这些将是一场噩梦。为了解决这个问题，我们可以在这里使用[用于 C 编程中的循环](https://www.tutorialgateway.org/for-loop-in-c-programming/)概念:
...
我们可以使用索引在 [C 编程](https://www.tutorialgateway.org/c-programming/)元素中访问二维数组。使用索引，我们可以分别访问或更改/改变数组中的每个元素。索引值从 0 开始，到 n-1 结束，其中 n 是行或列的大小。
