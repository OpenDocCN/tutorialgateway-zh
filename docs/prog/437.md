...
R 编程中的`odbcConnect()`函数在 SQL Server 和 R 之间建立连接很方便。这个`odbcConnect()`函数启动 SQL Server R 服务。`sqlQuery()`函数从 SQL 表中提取数据，并将数据存储在数据帧中。让我通过示例展示如何使用由 RODBC 库提供的 ODBC 连接和 SQL Server 本机客户端连接来设置 SQL Server R 服务。
