...
> 原文:[https://www.tutorialgateway.org/sql-where-clause/](https://www.tutorialgateway.org/sql-where-clause/)

SQL WHERE 子句用于限制选择语句返回的行数(或记录数)。这意味着选择语句只在 SQL Where 关键字后指定的条件为真时才返回记录。
...
*   条件:在这里，我们必须提供过滤器或条件。如果条件为真，则只有选择语句返回记录。
...
此子句并不限制选择语句选择的记录。事实上，在[更新语句](https://www.tutorialgateway.org/sql-update-statement/)中强制使用它是有用的。
