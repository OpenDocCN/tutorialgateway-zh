...
SQL Server 内连接、完全外部连接、左外部连接、右外部连接、自身连接和交叉连接的可视化表示为
...
左或左外部连接返回左表中的所有行和右表中的匹配行(如果有)。这个例子展示了如何写左外或左返回结果集。所以，参考 [SQL](https://www.tutorialgateway.org/sql/) 中的[左](https://www.tutorialgateway.org/sql-left-join/)篇。
