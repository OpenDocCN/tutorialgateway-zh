...
## Python 列表推导与Lambda 函数

如果更少的代码行是我们的优先事项，那么我们并不局限于此。可以使用[λ功能](https://www.tutorialgateway.org/python-lambda/)。这里，我们使用传统方法、列表推导方法和Lambda 函数将每个项目与它自己相乘。

通过查看代码，您可以看到Lambda 函数也非常有用。但是，代码易于阅读，执行速度也更快。
...
这是区分列表推导和Lambda 函数之间代码可读性的另一个例子。
