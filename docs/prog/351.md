...
此示例显示了使用Management Studio在 SQL Server 中创建视图所涉及的步骤。创建时要记住的限制是
...
让我们执行这个使用Management Studio设计的 Sql 视图查询，并查看结果
...
SQL Server 允许您使用内置存储过程 sp_rename 或Management Studio来重命名 SQL Server 中的视图。

### 在 SQL Management Studio中重命名
...
要在Management Studio中进行修改，请导航到要更改的工作室。接下来，右击它，选择设计选项
...
要使用Management Studio进行删除，请右键单击该名称，然后单击删除选项。对于这个演示，我们想删除
