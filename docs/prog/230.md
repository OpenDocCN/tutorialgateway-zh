...
> 原文:[https://www.tutorialgateway.org/python-random-array/](https://www.tutorialgateway.org/python-random-array/)
...
Python Numpy randn 函数生成一个、2D 和三维数组的随机数组。
...
这一次，我们使用 Numpy 随机 randn 函数生成二维和三维随机阵列。
...
让我们看看 Python Numpy 随机随机整数函数的例子
...
Python Numpy 随机正态函数根据正态分布生成随机数。这个 Numpy 标准接受数组的大小，然后用正态分布的值填充该数组。
...
Python Numpy 随机均匀函数生成随机数的均匀分布。这个 Numpy 随机均匀接受数组大小，并用均匀分布的值填充该数组。
