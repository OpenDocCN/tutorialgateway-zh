...
Python 数据帧分组依据函数类似于 [Sql Server](https://www.tutorialgateway.org/sql/) 中的[分组依据](https://www.tutorialgateway.org/sql-group-by-clause/)子句。我的意思是，您可以使用 Pandas`groupby()`函数按一些列对数据进行分组，并找到其他列的聚合结果。这是处理实时数据时的重要概念或功能之一。
