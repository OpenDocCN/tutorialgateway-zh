...
Python 中的 Pandas 数据帧是一种二维数据结构。这意味着，Pandas dataframe 以表格格式存储数据，即行和列。在本文中，我们展示了如何创建 Pandas 数据帧、访问、更改行和列。接下来，我们将讨论 Python 中的数据帧转换、行迭代等等。

## 如何用 Python 创建数据帧？

实时地，我们使用这个 Pandas 数据帧从 Sql Server、文本文件、Excel 文件或任何 CSV 文件中加载数据。接下来，我们按照我们的要求对数据进行切片和切割。一旦数据达到我们要求的格式，我们就使用 matplotlib 模块使用这些数据创建报告或图表。

### 在 Pandas 中创建一个空的数据帧

这是一个用 Python 创建空数据帧的简单示例。这里，我们正在创建一个空的
...
这里，我们创建一个 Python 整数值列表。接下来，我们用 Pandas。`DataFrame()`函数从列表创建我们的 df 或转换列表。
...
从混合列表创建或将混合列表转换为 Pandas 数据帧。这里，我们还使用了多行多列。
...
为了使用随机数创建 Pandas 数据帧，我们使用 [numpy](https://www.tutorialgateway.org/python-numpy-array/) 随机函数生成大小为 8 * 4 的[随机](https://www.tutorialgateway.org/python-random-array/)数。接下来，我们使用 [Python](https://www.tutorialgateway.org/python-tutorial/) 函数将这些序列转换成数据帧
...
### 巨蟒 Pandas 数据帧来自 dict

Pandas 允许你从字典或字典中创建数据帧。它非常直接。您所要做的就是声明一个不同值的字典，然后使用 Python `DataFrame()`函数将该字典转换为 DataFrame
...
要从字典创建 Pandas 数据帧，所有键的字典值的长度应该相同，否则会引发错误。接下来，如果您传递的是索引值，那么它们应该与键值或数组的长度相匹配，否则会引发错误。如果您没有传递任何索引值，那么它将自动为您创建一个索引，并从 0 到 n-1 开始。
...
让我再举一个 Pandas 数据帧的例子。这次，我们转换四列数据的字典键。
...
使用 Pandas 模块，您还可以创建一个包含一系列日期的数据帧。让我创建一个从 2019-01-01 到 2019-01-08 的日期。通过更改期间值，您可以生成更多数量的日期序列。
...
此示例显示了如何对列重新排序。默认情况下，数据帧将使用我们在实际数据中使用的列顺序。但是，您可以使用 this 参数来更改任何列的位置。让我把年龄从第二位改为第四位。
...
columns 属性以相同的顺序返回数据帧中可用列的列表以及数据类型。
...
Python 数据帧的可用 Pandas 属性列表

### Python 数据帧形状属性

Pandas 形状属性返回其中行数和列数的形状或元组。
...
以上 Pandas 的例子是返回一个对象类型的数组。这是因为，这两个数据帧都有混合内容(int、string)。如果不是这样，那么它就不会在数组中显示任何数据类型。为此，我们使用了一个整数数据帧
...
Python 数据帧索引和列具有名称属性，允许为索引或列分配名称。
...
同样，我们可以使用 Pandas dataframe 的列名属性来为列标题分配名称。
...
Python 数据帧中的数据以行和列的表格格式存储。这意味着，您可以使用列和行来访问项目。
...
Python 中的 Pandas 数据帧也可以使用行来访问。这里，我们使用索引切片技术从中返回所需的行。这里，data[1:]返回从索引 1 到 n-1 的所有行，data[1:3]返回从索引 1 到 3 的行。
...
Pandas loc 是需要理解的重要事情之一。您可以使用 loc[]一次选择多列和多行。或者，使用 Pandas 锁定[]选择一部分。使用此锁定选择行。
...
类似于 loc[]，Pandas DataFrame 有 iloc[]。但是，这将只接受整数值或索引来返回数据。
...
在本例中，我们将向您展示如何向现有数据帧添加新列。数据['Sale'] = [422.19，200.190，44.55]添加了一个名为 Sale 的全新列。数据['收入'] =数据['薪金'] +数据['基本']通过在薪金列和基本列中添加值来添加新列收入。
...
在 Python 中，有两种方法可以从 Pandas 数据帧中删除一列。您可以使用`del()`函数或`pop()`函数。在本例中，我们将使用这两个函数来删除其中的列。
...
在 Python 中，使用 Pandas 重命名功能来重命名一列或多列。在这里，我们使用 Pandas 重命名功能将职业列重命名为资格，将工资重命名为收入。
...
如果你是从 [R 语言](https://www.tutorialgateway.org/r-programming/)过来的，可能对头尾功能比较熟悉。`head()`函数接受整数值作为参数，并返回 Top 或第一个给定的记录数。
...
## 用 Python 转置 Pandas 数据帧

Python 数据帧具有转置矩阵的内置功能。为此，您必须使用 df。T
...
Python 数据帧`GROUP BY`函数类似于 [Sql Server](https://www.tutorialgateway.org/sql/) 中的[`GROUP BY`](https://www.tutorialgateway.org/sql-group-by-clause/)子句。我的意思是，您可以使用 Pandas`groupby()`函数按一些列对数据进行分组，并找到其他列的聚合结果。这是处理实时数据时的重要概念或功能之一。

在本例中，我们创建了一个不同列和数据类型的表。接下来，我们使用这个`groupby()`函数。第一个语句 data . group by(' professional ')。sum()按职业对数据帧列进行分组，并计算销售、薪资和年龄的总和。

第二种说法，data . group by([' professional '，' Age'])。sum()按职业和年龄列对数据帧进行分组，并计算销售额和工资的总和。请记住，任何字符串列(无法聚合)都将被连接或组合。
...
Pandas 堆栈函数用于压缩数据帧对象的一个级别。为了使用这个堆栈函数，您可以简单地调用 data_to_stack.stack()。在这个例子中，我们对分组数据(`groupby()`函数结果)使用这个 Python DataFrame 堆栈函数来进一步压缩它。
...
Pandas 数据帧连接函数用于组合或连接对象。首先，我们声明了两个大小为 4 * 6 的随机值的数据帧。接下来，我们使用`concat()`函数来连接数据帧
...
在上面的例子中，我们连接了两个相同大小的 df 对象。但是，您可以使用这个 Pandas DataFrame`concat()`函数来连接或组合两个以上不同大小的对象。
...
在这个例子中，我们使用了一些 Pandas 数据帧数学函数。为了这个数学运算演示的目的，我们正在寻找每一列和每一行的平均值和中值。要获得每行的平均值或中值，必须在函数中放置整数 1。
...
如果数据帧中的值为空，则为空，否则为假。如果值不为空，Pandas`notnull()`函数返回 True，否则返回 False。
...
我们也可以用有意义的数字替换那些空值。所以，要替换 Pandas 中的空值，可以使用 Python DataFrame`fillna()`函数或`replace()`函数。
...
要将 Pandas 数据帧中的数据加载到 csv 文件或文本文件中，必须使用 Pandas`to_csv()`函数。
...
在 Python 中，使用 iteritems、iterrows 和 itertuple 三个函数中的任何一个来遍历行并返回每一行。更多信息请参考[Pandas](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.html)模块。
