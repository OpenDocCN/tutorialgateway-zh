...
在本例中，我们使用本地目录中 CSV 文件的数据。从下面的 [Python](https://www.tutorialgateway.org/python-tutorial/) 代码中可以看到，首先，我们使用Pandas[数据帧](https://www.tutorialgateway.org/python-pandas-dataframe/)`groupby()`函数对区域项目进行分组。接下来，我们找到销售金额的总和。接下来，我们将地区名称与销售额相比较。这意味着下面的 matplotlib 条形图将显示所有地区的销售额。
