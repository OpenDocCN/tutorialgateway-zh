...
此示例说明如何使用；[`SELECT`语句](https://www.tutorialgateway.org/sql-select-statement/)并在存储过程中插入语句。从下面的代码片段中，您可以看到我们正在使用[`INSERT SELECT`语句](https://www.tutorialgateway.org/sql-insert-into-select-statement/)将员工表中的所有记录插入到员工数据表中
