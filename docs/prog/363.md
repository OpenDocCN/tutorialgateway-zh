...
我们可以用两种方式创造它们。在本例中，我们向您展示了使用Management Studio(SSMS)创建触发器的步骤。在我们开始生成之前，让我们看看我们的数据库是否有任何现有的。
...
以下示例将帮助您理解使用Management Studio修改现有 SQL 触发器和查询所涉及的步骤。

在我们开始修改 SQL 触发器之前，让我向您展示Management Studio向您提供的可用选项列表。如下所示，您可以删除它，删除并重新创建它，更改它，并创建它。

![Triggers in SQL Server 10](img/15a132e1e370aa7a6b613d82cb546b90.png)

要使用Management Studio修改它，请导航到保存它们的表。接下来，选择您想要修改的(触发器 _in_sql)，右键单击它将打开上下文菜单。在此，选择修改选项
...
以下示例将向您展示如何使用查询和Management Studio(SSMS)删除它们。在这里，我们向您展示如何使用 SSMS 删除它们。要在使用 SSMS 时删除，右键单击要删除的名称将打开上下文菜单。您可以单击删除选项
