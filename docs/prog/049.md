...
例如 printf()、scanf()、strcpy()等。是 C 编程语言中的一些内置函数。以下是 C 程序设计中函数的优点
...
函数声明是可选的。众所周知，C 编程从 main()开始执行。因此，当编译器到达方法调用时，它将向上运行以检查定义。意思是，
...
注意:用户定义的函数名应与 [C 编程](https://www.tutorialgateway.org/c-programming/)中的调用名完全匹配。
